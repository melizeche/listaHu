...
class ListaUnicaViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        query = DenunciaFilter(request.GET, queryset=queryset)
        serializer = DenunciaSerializer(query.qs, many=True)
        return Response(serializer.data)
...
def home(request: HttpRequest):
    ...
    return render(
        request, "index.html", {"form": form, "denuncias": cant, "fail": fail}
    )
            ...
            for denuncia in query:
                withthumbs.append(
                    (
                        ...
                        "%s_th%s"
                        % (
                            path.splitext(str(denuncia.screenshot))[0],
    ...
    if "formato" in kwargs:
        ...
        if formato == "csv":
            ...
            response["Content-Disposition"] = (
                'attachment; filename="LISTA_HU_%s.csv"'
        ...
        # vCard all
            ...
            response["Content-Disposition"] = (
                'attachment; filename="LISTA_HU_%s.VCF"'
                % (time.strftime("%Y-%m-%d", time.localtime()))
            )
            return response
        # vCard with more than 1 denuncia
            ...
            lista = (
                Denuncia.objects.values("numero")
                .annotate(the_count=Count("numero"))
                .order_by("-the_count")
                .filter(the_count__gte=2)
            )
            vc = str(vcard("Todos", lista))
            response = HttpResponse(vc, content_type="text/vcard")
            response["Content-Disposition"] = (
                'attachment; filename="LISTA_HU_REP_%s.VCF"'
                % (time.strftime("%Y-%m-%d", time.localtime()))
            )
            return response
        # vCard just estafa y extorsión
            ...
            lista = (
                Denuncia.objects.filter(activo=True)
                .distinct("numero")
                .exclude(tipo=Tipo.objects.get(titulo="SPAM").id)
            )
            vc = str(vcard("Todos", lista))
            response = HttpResponse(vc, content_type="text/vcard")
            response["Content-Disposition"] = (
                'attachment; filename="LISTA_HU_NOSPAM_%s.VCF"'
                % (time.strftime("%Y-%m-%d", time.localtime()))
            )
            return response
